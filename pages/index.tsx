import {
	AcsCarousel,
	AcsCheckboxCard, AcsCheckboxMultiple, AcsDrawer, AcsDropzone,
	...
	AcsInputText,
	AcsInputTextArea,
	AcsInputTime, AcsModal,
	AcsPaginate,
	AcsPaginateSecondary, AcsPopover, AcsRadioGroup, AcsRadioSelectCard,
	AcsRangeSlider,
	AcsSelect,
	AcsSelectCreate,
	AcsSelectMultiple,
...
const Home: NextPage = () => {
	...
	const [treeItem, setTreeItem] = useState<{ id: string | number | null; name: string | null }>({ id: null, name: null })
	...
	const [selectedCheckboxIds, setSelectedCheckboxIds] = useState<(string | number)[]>([])
	...
	const checkboxCard = [
		...
		{
			id: 3,
			body: <Box width={"200px"} height={"auto"}>
				...
				<Text textAlign={"right"} fontSize={"xs"}>created by ORCEL</Text>
			</Box>
		},
		{
			id: 4,
	...
	return (
		<Box width={'100vw'} px={8} py={2}>
			...
			<SimpleGrid columns={2} spacing={10}>
				...
				<AcsPaginate max={100} current={paginate} handleChangePage={setPaginate} withPageInfos />
				<AcsSlider allStep onChange={setSlider} value={slider} label={"Slider"}  indicatorSteps={10} />
				<AcsPaginateSecondary max={100} current={paginate} handleChangePage={ setPaginate }  />
				<AcsTree tree={tree} item={treeItem} setItem={setTreeItem} />
				<AcsCheckboxCard contentArray={checkboxCard} selectedCardIds={checked} setSelectedCardIds={setChecked} />
				<AcsRadioSelectCard contentArray={checkboxCard} selectedCardId={radioChecked} setSelectedCardId={setRadioChecked} />
				<Button onClick={() => setModalDrawer(!modalDrawer)}>
					{modalDrawer ? "Fermer" : "Ouvrir"}
				</Button>
				<AcsDrawer  title={"Drawer"}
							isOpen={modalDrawer}
				...
				/>
				<AcsRadioGroup
					handleChange={(radio) => setRadio(radio)}
					valuesArray={radioArray}
					value={radio}
				/>
				<AcsCheckboxMultiple
					 multiple contentArray={contentArray}
